...
### メモリ効率
- **原則: ディスクベース処理** — データは取得したらすぐDBに書き込み、メモリに保持しない
- バッチ単位で処理し、バッチ完了後にメモリを解放（全件をメモリに溜めない）
- スナップショット生成は1課題ずつDBから読み出して処理し、結果を即座にDBへ書き戻す
- 検索結果・クエリ結果はページネーションで必要分だけ取得
- フル同期時のソフトデリート判定はDBのクエリ（`NOT IN` サブクエリ）で行い、全キーをメモリに持たない
- インメモリ処理を許容する場面:
  - メタデータ（ステータス、優先度等）: 件数が少なく（数十件程度）一括処理が効率的
  - 1課題内の変更履歴: スナップショット構築時に1課題分のみ保持
  - 設定ファイル（settings.json）: サイズが小さく常時読み書きが必要
