...
- [x] **T-120** ハンドラーテスト（httptest + サービスモック）
- [x] **T-121** SSE 進捗通知テスト
- [x] **T-122** CORS・ミドルウェアテスト
...
- [x] **T-130** SvelteKit プロジェクト初期化（`web/`）
  - Svelte 5, TypeScript, Tailwind CSS
- [x] **T-131** Vite プロキシ設定（`/api` → Go サーバー）
- [x] **T-132** `web/src/lib/api/client.ts` — API クライアント
  - 全エンドポイント対応（POST + SSE）
- [x] **T-133** `web/src/lib/types.ts` — 型定義（Go models に対応）
- [x] **T-134** `web/src/lib/stores/` — Svelte stores
  - projects, syncProgress, settings

## Phase 14: Svelte ページ実装

- [x] **T-140** 共通レイアウト + ナビゲーション
- [x] **T-141** ダッシュボード（同期状態、プロジェクト概要）
- [x] **T-142** プロジェクト管理ページ（一覧、有効/無効切替）
- [x] **T-143** 同期ページ（実行ボタン、SSE リアルタイム進捗表示）
- [x] **T-144** 課題検索ページ（フィルタ、リスト/ボード切替）
- [x] **T-145** SQL クエリページ（エディタ、スキーマブラウザ、結果テーブル）
- [x] **T-146** 設定ページ（JIRA接続、DB、同期設定）

## Phase 15: ビルド統合

- [x] **T-150** Go サーバーに Svelte ビルド成果物を埋め込み（`embed` + build tag）
- [x] **T-151** 本番ビルドスクリプト（Makefile）
